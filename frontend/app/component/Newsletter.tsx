    ...
    return (
        <section className="py-12 lg:py-16 bg-white">
            <div className="container-torano">
                <div className="relative rounded-3xl bg-[#0a192f] text-white p-8 sm:p-12 lg:p-14 overflow-hidden shadow-xl border border-slate-800">
                    {/* Subtle glow accent */}
                    <div className="absolute -right-20 -bottom-20 w-80 h-80 bg-[#1e40af]/30 rounded-full blur-3xl pointer-events-none" />

                    <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                        {/* Left Column: Heading & Description */}
                        <div className="lg:col-span-6 text-left">
                            <span className="text-xs font-bold text-[#c9a227] uppercase tracking-[0.25em] block mb-2">
                                Đặc Quyền Hội Viên
                            </span>
                            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight text-white uppercase">
                                Đăng Ký Nhận Tin
                            </h2>
                            <p className="mt-2.5 text-sm sm:text-base text-slate-300 font-normal leading-relaxed max-w-lg">
                                Nhận ngay voucher giảm 10% cho đơn hàng đầu tiên và cập nhật sớm nhất các bộ sưu tập giới hạn từ HAVEN.
                            </p>
                        </div>

                        {/* Right Column: Clean White Form Pill */}
                        <div className="lg:col-span-6">
                            <form
                                onSubmit={handleSubmit}
                                className="flex flex-col sm:flex-row items-center gap-2 bg-white p-2 rounded-2xl sm:rounded-full shadow-2xl transition-all"
                            >
                                ...
                                <motion.button
                                    ...
                                    whileTap={{ scale: 0.98 }}
                                    className="w-full sm:w-auto px-7 py-3.5 bg-[#0a192f] hover:bg-[#1e40af] text-white rounded-xl sm:rounded-full text-xs font-black tracking-wider uppercase whitespace-nowrap transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                                >
