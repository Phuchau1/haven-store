...
export default function AdminShippingMethodsPage() {
    ...
    const openModal = (item?: ShippingMethod) => {
        if (item) {
            setEditingItem(item);
            setFormData({ ...item });
        } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const url = editingItem ? `/api/admin/extra/shipping-methods?id=${editingItem.id}` : '/api/admin/extra/shipping-methods';
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const res = await fetch(`/api/admin/extra/shipping-methods?id=${id}`, { method: 'DELETE' });
            const data = await res.json();
